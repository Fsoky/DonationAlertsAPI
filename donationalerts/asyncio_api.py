...
class DonationAlertsAPI:
    def __init__(self, client_id, client_secret, redirect_uri, scopes):
        ...
        for symbol in symbols:
            if symbol in scopes:
                self.scope = scopes.replace(
                    symbol, "%20"
                ).strip()  # Replaces some symbols on '%20' for stable work
            else:
                self.scope = scopes
    ...
    async def get_access_token(self, code, *, full_json=False):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{DEFAULT_URL}token", data=payload
            ) as response:
        ...
        async with websockets.connect(
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.post(
                    ...
                    obj = json.loads(await websocket.recv())["result"]["data"][
                        "data"
                    ]
