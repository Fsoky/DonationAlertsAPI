...
class DonationAlertsAPI:
    ...
    def donations_list(self, access_token, *, page: int = 1):
        ...
        objs = requests.get(
            f"{DEFAULT_API_LINK}alerts/donations?page={page}", headers=headers
        ).json()
    ...
    def user(self, access_token):
        ...
        obj = requests.get(
            f"{DEFAULT_API_LINK}user/oauth", headers=headers
        ).json()
    ...
    def send_custom_alert(
        ...
        *,
        ...
        obj = requests.post(
            f"{DEFAULT_API_LINK}custom_alert", data=payload, headers=headers
        ).json()
        return obj
    ...
    def get_refresh_token(self, access_token, refresh_token):
        ...
        obj = requests.post(
            f"{DEFAULT_URL}token", data=payload, headers=headers
        ).json()
        return Data(
            obj["access_token"],
            obj["expires_in"],
...
class Centrifugo:
    ...
    def subscribe(self, channels):
        ...
        ws.send(
            json.dumps(
                {
                    "params": {"token": self.socket_connection_token},
                    "id": self.user_id,
                }
            )
        )
        ...
        data = {"channels": chnls, "client": ws_response["result"]["client"]}

        response = requests.post(
            f"{DEFAULT_API_LINK}centrifuge/subscribe",
